...
func TestBetterGetRule(t *testing.T) {
	tables := []struct {
		...
		{"702.22.", "<b>702.22c.</b> As a player declares attackers, they may declare that one or more attacking creatures with banding and up to one attacking creature without banding (even if it has “bands with other”) are all in a “band.” They may also declare that one or more attacking [quality] creatures with “bands with other [quality]” and any number of other attacking [quality] creatures are all in a band. A player may declare as many attacking bands as they want, but each creature may be a member of only one of them. (Defending players can’t declare bands but may use banding in a different way; see rule 702.22j.)"},
		{"205.3m.", "<b>205.3m.</b> <i>[This subtype list is too long for chat. Please see https://yawgatog.com/resources/magic-rules/#R2053m ]</i>"},
...
func TestGetRule(t *testing.T) {
	tables := []struct {
		...
		{"rule 701.29", "<b>701.29a.</b> To “fateseal N” means to look at the top N cards of an opponent’s library, then put any number of them on the bottom of that library in any order and the rest on top of that library in any order."},
		...
		{"ex 1 111.3", "<b>[111.3] Example:</b> Jade Mage has the ability “{2}{G}: Create a 1/1 green Saproling creature token.” The resulting token has no mana cost, supertypes, rules text, or abilities."},
		{"ex 111.3 1", "<b>[111.3] Example:</b> Jade Mage has the ability “{2}{G}: Create a 1/1 green Saproling creature token.” The resulting token has no mana cost, supertypes, rules text, or abilities."},
		{"ex 999.99", "Example not found"},
		{"example 999.99", "Example not found"},
		{"example 1 999.99", "Example not found"},
		{"example 999.99 1", "Example not found"},
		{"ex 104.3a", "Example not found"},
		{"ex 1 104.3a", "Example not found"},
		{"ex 104.3a 1", "Example not found"},
		...
		{"ex 2 112.4", "Too few examples for rule found (wanted 2, got 1)"},
		{"ex 112.4 2", "Too few examples for rule found (wanted 2, got 1)"},
